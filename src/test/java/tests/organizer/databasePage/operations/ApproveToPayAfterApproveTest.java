...
public class ApproveToPayAfterApproveTest {
    @Test(priority = 1)
    ...
    @Test(priority = 2)
    public static void statusIsApprovedToPayAfterApproveToPay() throws InterruptedException {
        Go.waitIfVisible(By.xpath("//nb-spinner[@class='size-medium status-basic ng-star-inserted']"));
        getApproveToPayBtn().click();
        Thread.sleep(2000);
        Assert.assertEquals(RegistrantInfoPOM.getApprovalStatus().getText(),Setup.planningTestData.getPendingPaymentText());
    }
}
